...
public class AddressResource {
    ...
    @Timed
    public ResponseEntity<Void> create(@RequestBody Address address) throws URISyntaxException {
        log.debug("REST request to save Address : {}", address);
        if (address.getId() != null) {
            return ResponseEntity.badRequest().header("Failure", "A new address cannot already have an ID").build();
        }
        addressRepository.save(address);
        return ResponseEntity.created(new URI("/api/addresss/" + address.getId())).build();
    }
    ...
    @RequestMapping(value = "/addresss",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Void> update(@RequestBody Address address) throws URISyntaxException {
